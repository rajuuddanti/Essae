...
class MainVm(
    ...
    var storePricesByPlu by mutableStateOf(
                ...
                onSuccess = { p ->
                    ...
                    if (p.role == "ADMIN") {
                        ...
                        api.getPlus().onSuccess { rows ->
                            cloudPlusIds =
                                rows.associate { it.number to it.id }
                        }

                        api.getStorePluPrices().onSuccess { rows ->
                            storePricesByPlu =
                                rows
                                    .groupBy { it.pluId }
                                    .mapValues { (_, prices) ->
                                        prices.associate {
                                            it.storeId to it.unitPrice
                                        }
                                    }
                        }
                    } else {
